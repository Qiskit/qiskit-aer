...
class AerJobSet(Job):
    ...
    @requires_submit
    def status(
        self, worker: Union[None, int, Iterable[int]]
    ...
    def _get_worker_result(self, worker: int, timeout: Optional[float] = None):
        ...
        try:
            ...
            if result is None or not result.success:
                if result:
                    logger.warning(
                        "AerJobSet %s Error: %s", aer_job.name(), result.header
                    )
                else:
                    logger.warning(
                        "AerJobSet %s did not return a result", aer_job.name()
                    )
        except JobError:
            raise JobError(
    ...
    def _accumulate_experiment_results(self, results: List[Result]):
        ...
        for each_result in results:
            ...
            for _result in each_result.results:
                if not hasattr(_result.data, "counts") and not hasattr(
                    _result.data, "memory"
                ):
    ...
    def _combine_results(self, results: List[Union[Result, None]] = None) -> Result:
        ...
        for each_result in results:
            if each_result is not None:
                combined_result["results"].extend(
                    x.to_dict() for x in each_result.results
                )
    ...
    @requires_submit
    def job(
        self, experiment: Union[str, QuantumCircuit, Schedule]
    ) -> Tuple[AerJob, int]:
