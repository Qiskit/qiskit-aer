...
const Operations::OpSet StateOpSet(
    ...
    {"u1",     "u2",      "u3",  "u",    "U",    "CX",   "cx",   "cz",
     ...
     "csx", "mcsx", "csxdg", "mcsxdg",  "delay", "pauli", "mcx_gray", "cu", "mcu", "mcp","ecr"});
...
protected:
  ...
  void apply_diagonal_matrix(const reg_t &qubits, const cvector_t<double> & diag); 

  // Apply a vector of control matrices to given qubits (identity on all other
  // qubits)
  void apply_multiplexer(
  ...
  void apply_kraus(const reg_t &qubits, const std::vector<cmatrix_t> &krausops, RngEngine &rng);
...
//=========================================================================
template <class tensor_net_t>
void State<tensor_net_t>::apply_op(
...
template <class tensor_net_t>
void State<tensor_net_t>::apply_save_statevector_dict(const Operations::Op &op,
                                                   ExperimentResult &result) 
{
  if (op.qubits.size() != BaseState::qreg_.num_qubits()) {
    throw std::invalid_argument(
...
void State<tensor_net_t>::apply_gate(const Operations::Op &op) 
{
  ...
  switch (it->second) {
    ...
    case Gates::mcr:
      ...
      break;
    case Gates::mcrx:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::x, std::real(op.params[0]));
      break;
    case Gates::mcry:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::y, std::real(op.params[0]));
      break;
    case Gates::mcrz:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::z, std::real(op.params[0]));
      break;
    case Gates::rxx:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::xx, std::real(op.params[0]));
      break;
    case Gates::ryy:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::yy, std::real(op.params[0]));
      break;
    case Gates::rzz:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::zz, std::real(op.params[0]));
      break;
    case Gates::rzx:
      BaseState::qreg_.apply_rotation(op.qubits, TensorNetwork::Rotation::zx, std::real(op.params[0]));
      break;
    case Gates::ecr:
...
template <class tensor_net_t>
void State<tensor_net_t>::apply_diagonal_matrix(const reg_t &qubits, const cvector_t<double> & diag)
{
...
template <class tensor_net_t>
void State<tensor_net_t>::apply_measure(const reg_t &qubits, const reg_t &cmemory,
                                      const reg_t &cregister, RngEngine &rng) 
{
...
template <class tensor_net_t>
void State<tensor_net_t>::initialize_from_vector(const cvector_t<double> &params)
{
  BaseState::qreg_.initialize();
