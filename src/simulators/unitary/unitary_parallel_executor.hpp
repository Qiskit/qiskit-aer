...
void ParallelExecutor<unitary_matrix_t>::initialize_qreg(uint_t num_qubits) 
{
  ...
  for(iChunk=0;iChunk<BaseExecutor::states_.size();iChunk++){
    BaseExecutor::states_[iChunk].qreg().set_num_qubits(BaseExecutor::chunk_bits_);
  }
...
#pragma omp parallel for private(iChunk) 
    for(int_t ig=0;ig<BaseExecutor::num_groups_;ig++){
      for(iChunk = BaseExecutor::top_chunk_of_group_[ig];iChunk < BaseExecutor::top_chunk_of_group_[ig + 1];iChunk++){
        uint_t irow,icol;
...
template <class unitary_matrix_t>
void ParallelExecutor<unitary_matrix_t>::apply_parallel_op(const Operations::Op &op,
                                                 ExperimentResult &result,
...
template <class unitary_matrix_t>
void ParallelExecutor<unitary_matrix_t>::apply_save_unitary(const Operations::Op &op,
                                                 ExperimentResult &result,
                                                 bool last_op) 
{
  ...
  if (last_op) {
    result.save_data_pershot(BaseExecutor::states_[0].creg(), key, move_to_matrix(),
                                 Operations::OpType::save_unitary,
                                 op.save_type);
  } else {
    result.save_data_pershot(BaseExecutor::states_[0].creg(), key, copy_to_matrix(),
                                 Operations::OpType::save_unitary,
                                 op.save_type);
  }
}
...
template <class unitary_matrix_t>
double  ParallelExecutor<unitary_matrix_t>::expval_pauli(const reg_t &qubits,
