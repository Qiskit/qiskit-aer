...
public:
  ...
  virtual void add_metadata(ExperimentResult &data) const {
  }
  ...
  void add_creg_to_data(ExperimentResult &data) const;
  ...
  void snapshot_state(const Operations::Op &op, ExperimentResult &data,
                      std::string name = "") const;

  // Snapshot the classical memory bits state (single-shot)
  void snapshot_creg_memory(const Operations::Op &op, ExperimentResult &data,
                            std::string name = "memory") const;

  // Snapshot the classical register bits state (single-shot)
  void snapshot_creg_register(const Operations::Op &op, ExperimentResult &data,
...
void State<state_t>::snapshot_state(const Operations::Op &op,
  ...
  data.data.add_pershot_snapshot(name, op.string_params[0], qreg_);
}
...
void State<state_t>::snapshot_creg_memory(const Operations::Op &op,
                                          ExperimentResult &data,
                                          std::string name) const {
  data.data.add_pershot_snapshot(name,
                               op.string_params[0],
                               creg_.memory_hex());
}
...
template <class state_t>
void State<state_t>::add_creg_to_data(ExperimentResult &data) const {
  if (creg_.memory_size() > 0) {
    std::string memory_hex = creg_.memory_hex();
    data.data.add_memory_count(memory_hex);
    data.data.add_pershot_memory(memory_hex);
  }
  // Register bits value
  if (creg_.register_size() > 0) {
    data.data.add_pershot_register(creg_.register_hex());
  }
}
//-------------------------------------------------------------------------
} // end namespace Base
