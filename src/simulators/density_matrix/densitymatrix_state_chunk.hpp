...
const Operations::OpSet StateOpSet(
  ...
  {"U", "CX", "u1", "u2", "u3", "cx", "cz", "swap", "id", "x", "y",
    "z", "h", "s", "sdg", "t", "tdg", "ccx", "swap_chunk"},
  // Snapshots
...
void State<densmat_t>::initialize_qreg(uint_t num_qubits) 
{
  ...
  if(BaseState::qregs_[0].name() == "density_matrix_gpu" && omp_get_num_threads() == 1){
  ...
  for(i=0;i<BaseState::num_local_chunks_;i++){
    BaseState::qregs_[i].chunk_setup(BaseState::chunk_bits_,num_qubits*2,i + BaseState::global_chunk_index_,BaseState::num_local_chunks_);
...
void State<densmat_t>::apply_ops(const std::vector<Operations::Op> &ops,
  ...
  while(iOp < nOp){
    ...
    if (BaseState::creg_.check_conditional(ops[iOp]) == false)
      return;
    switch (ops[iOp].type) {
...
void State<densmat_t>::apply_gate(const uint_t iChunk, const Operations::Op &op) {
  ...
  switch (it -> second) {
    ...
    case Gates::u2:
      ...
      break;
    case Gates::u1:
      apply_gate_phase(iChunk, op.qubits[0], std::exp(complex_t(0., 1.) * op.params[0]));
      break;
    case Gates::cx:
      BaseState::qregs_[iChunk].apply_cnot(op.qubits[0], op.qubits[1]);
      break;
    case Gates::cz:
      BaseState::qregs_[iChunk].apply_cz(op.qubits[0], op.qubits[1]);
      break;
    case Gates::id:
      break;
    case Gates::x:
    ...
    case Gates::y:
      ...
      break;
    case Gates::z:
      BaseState::qregs_[iChunk].apply_z(op.qubits[0]);
      break;
    case Gates::h:
...
template <class densmat_t>
void State<densmat_t>::apply_gate_u3(const uint_t iChunk, uint_t qubit, double theta, double phi, double lambda) {
  BaseState::qregs_[iChunk].apply_unitary_matrix(reg_t({qubit}), Utils::VMatrix::u3(theta, phi, lambda));
}

template <class densmat_t>
void State<densmat_t>::apply_gate_phase(const uint_t iChunk, uint_t qubit, complex_t phase) {
  cvector_t diag = {{1., phase}};
  BaseState::qregs_[iChunk].apply_diagonal_unitary_matrix(reg_t({qubit}), diag);
}
