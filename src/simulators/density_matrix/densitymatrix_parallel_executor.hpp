...
#pragma omp parallel for 
    for(int_t ig=0;ig<BaseExecutor::num_groups_;ig++){
      for(int_t iChunk = BaseExecutor::top_chunk_of_group_[ig];iChunk < BaseExecutor::top_chunk_of_group_[ig + 1];iChunk++){
        if(BaseExecutor::global_chunk_index_ + iChunk == 0){
    ...
    for(int_t i=0;i<BaseExecutor::states_.size();i++){
      if(BaseExecutor::global_chunk_index_ + i == 0){
...
void ParallelExecutor<densmat_t>::initialize_from_vector(const list_t &vec)
{
  ...
  else if((1ull << (BaseExecutor::num_qubits_*2)) == vec.size() * vec.size()) {
    int_t iChunk;
    if(BaseExecutor::chunk_omp_parallel_ && BaseExecutor::num_groups_ > 0){
#pragma omp parallel for 
      for(int_t ig=0;ig<BaseExecutor::num_groups_;ig++){
        for(int_t iChunk = BaseExecutor::top_chunk_of_group_[ig];iChunk < BaseExecutor::top_chunk_of_group_[ig + 1];iChunk++){
          uint_t irow_chunk = ((iChunk + BaseExecutor::global_chunk_index_) >> ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_))) << (BaseExecutor::chunk_bits_);
          uint_t icol_chunk = ((iChunk + BaseExecutor::global_chunk_index_) & ((1ull << ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_)))-1)) << (BaseExecutor::chunk_bits_);
          ...
          for(i=0;i<(1ull << BaseExecutor::chunk_bits_);i++){
            ...
            vec2[i] = std::conj(vec[(icol_chunk << BaseExecutor::chunk_bits_) + i]);
          }
          BaseExecutor::states_[iChunk].qreg().initialize_from_vector(AER::Utils::tensor_product(vec1, vec2));
        }
      }
    }
    else{
    ...
    throw std::runtime_error("DensityMatrixChunk::initialize input vector is incorrect length. Expected: " +
                             std::to_string((1ull << (BaseExecutor::num_qubits_*2))) + " Received: " +
...
template <class densmat_t>
void ParallelExecutor<densmat_t>::apply_save_amplitudes_sq(const Operations::Op &op,
                                                ExperimentResult &result) 
{
...
#pragma omp parallel for if(BaseExecutor::chunk_omp_parallel_) private(iChunk) 
  for(iChunk=0;iChunk<BaseExecutor::states_.size();iChunk++){
    uint_t irow,icol;
    irow = (BaseExecutor::global_chunk_index_ + iChunk) >> ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_));
    icol = (BaseExecutor::global_chunk_index_ + iChunk) - (irow << ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_)));
    if(irow != icol)
      continue;

    for (int_t i = 0; i < size; ++i) {
      uint_t idx = BaseExecutor::mapped_index(op.int_params[i]);
      if(idx >= (irow << BaseExecutor::chunk_bits_) && idx < ((irow+1) << BaseExecutor::chunk_bits_))
      amps_sq[i] = BaseExecutor::states_[iChunk].qreg().probability(idx - (irow << BaseExecutor::chunk_bits_));
    }
  }
#ifdef AER_MPI
...
double ParallelExecutor<densmat_t>::expval_pauli(const reg_t &qubits,
  ...
  int_t nrows = 1ull << ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_));
  ...
  if(qubits_out_chunk.size() > 0){  //there are bits out of chunk
      ...
      for(i=0;i<nrows/2;i++){
        ...
        if(BaseExecutor::chunk_index_begin_[BaseExecutor::distributed_rank_] <= iChunk && BaseExecutor::chunk_index_end_[BaseExecutor::distributed_rank_] > iChunk){  //on this process
          ...
          if (z_mask && (AER::Utils::popcount(irow & z_mask) & 1))
            sign = -2.0;
          expval += sign * BaseExecutor::states_[iChunk-BaseExecutor::global_chunk_index_].qreg().expval_pauli_non_diagonal_chunk(qubits_in_chunk, pauli_in_chunk,phase);
        }
      }
    }
    else{
      for(i=0;i<nrows;i++){
        ...
        if(BaseExecutor::chunk_index_begin_[BaseExecutor::distributed_rank_] <= iChunk && BaseExecutor::chunk_index_end_[BaseExecutor::distributed_rank_] > iChunk){  //on this process
          ...
          if (z_mask && (AER::Utils::popcount(i & z_mask) & 1))
            sign = -1.0;
          expval += sign * BaseExecutor::states_[iChunk-BaseExecutor::global_chunk_index_].qreg().expval_pauli(qubits_in_chunk, pauli_in_chunk,1.0);
        }
      }
    }
  }
    ...
    for(i=0;i<nrows;i++){
      uint_t iChunk = i * (nrows+1);
      if(BaseExecutor::chunk_index_begin_[BaseExecutor::distributed_rank_] <= iChunk && BaseExecutor::chunk_index_end_[BaseExecutor::distributed_rank_] > iChunk){  //on this process
        expval += BaseExecutor::states_[iChunk-BaseExecutor::global_chunk_index_].qreg().expval_pauli(qubits, pauli,1.0);
      }
    }
  }
...
template <class densmat_t>
void ParallelExecutor<densmat_t>::apply_save_density_matrix(const Operations::Op &op,
                                                 ExperimentResult &result,
                                                 bool last_op) 
{
  result.save_data_average(BaseExecutor::states_[0].creg(), op.string_params[0],
                                        ...
                                        bool last_op) 
{
  ...
  std::string key = (op.string_params[0] == "_method_")
                      ...
                      : op.string_params[0];
  if (last_op) {
    result.save_data_average(BaseExecutor::states_[0].creg(), key, move_to_matrix(),
                             Operations::OpType::save_densmat, save_type);
  } else {
    result.save_data_average(BaseExecutor::states_[0].creg(), key, copy_to_matrix(),
                             Operations::OpType::save_densmat, save_type);
  }
}
...
template <class densmat_t>
cmatrix_t ParallelExecutor<densmat_t>::reduced_density_matrix(const reg_t& qubits, bool last_op) 
{
  ...
  if (qubits.empty()) {
    ...
    if ((qubits.size() == BaseExecutor::num_qubits_) && (qubits == qubits_sorted)) {
      if (last_op) {
...
template <class densmat_t>
cmatrix_t ParallelExecutor<densmat_t>::reduced_density_matrix_helper(const reg_t &qubits,
                                          const reg_t &qubits_sorted) 
{
  int_t iChunk;
  ...
  size_t size_required = (sizeof(std::complex<double>) << (qubits.size()*2)) + (sizeof(std::complex<double>) << (BaseExecutor::chunk_bits_*2))*BaseExecutor::num_local_chunks_;
  ...
  if(BaseExecutor::distributed_rank_ == 0){
    ...
    for(iChunk=0;iChunk<BaseExecutor::num_global_chunks_;iChunk++){
      int_t i;
      uint_t irow_chunk = (iChunk >> ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_))) << BaseExecutor::chunk_bits_;
      uint_t icol_chunk = (iChunk & ((1ull << ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_)))-1)) << BaseExecutor::chunk_bits_;

      if(iChunk < BaseExecutor::num_local_chunks_)
...
#pragma omp critical
    ...
    for(iChunk=0;iChunk<BaseExecutor::num_global_chunks_;iChunk++){
      ...
      if(iProc == BaseExecutor::distributed_rank_){
        auto tmp = BaseExecutor::states_[iChunk-BaseExecutor::global_chunk_index_].qreg().copy_to_matrix();
...
template <class densmat_t>
void ParallelExecutor<densmat_t>::apply_measure(const reg_t &qubits, const reg_t &cmemory,
                                     const reg_t &cregister, RngEngine &rng) 
{
...
template <class densmat_t>
rvector_t ParallelExecutor<densmat_t>::measure_probs(const reg_t &qubits) const 
{
...
#pragma omp parallel for private(i,j,k)
    for(int_t ig=0;ig<BaseExecutor::num_groups_;ig++){
      for(i = BaseExecutor::top_chunk_of_group_[ig];i < BaseExecutor::top_chunk_of_group_[ig + 1];i++){
        uint_t irow,icol;
        irow = (BaseExecutor::global_chunk_index_ + i) >> ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_));
        icol = (BaseExecutor::global_chunk_index_ + i) - (irow << ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_)));
...
#pragma omp parallel for 
      for(int_t ig=0;ig<BaseExecutor::num_groups_;ig++){
        for(int_t i = BaseExecutor::top_chunk_of_group_[ig];i < BaseExecutor::top_chunk_of_group_[ig + 1];i++)
          BaseExecutor::states_[i].qreg().apply_diagonal_unitary_matrix(qubits, mdiag);
      }
    }
    else{
      for(int_t i=0;i<BaseExecutor::states_.size();i++)
...
#pragma omp parallel for 
      ...
      if(qubits_out_chunk.size() > 0){  //out of chunk exchange
        for(int_t i=0;i<qubits_out_chunk.size();i++){
          ...
          BaseExecutor::apply_chunk_x(qubits_out_chunk[i]+(BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_));
        }
      }
    }
  }
...
template <class densmat_t>
std::vector<reg_t> ParallelExecutor<densmat_t>::sample_measure(const reg_t &qubits,
                                                    uint_t shots,
                                                    RngEngine &rng) const
{
...
#pragma omp parallel for private(i) 
  ...
  for(i=0;i<BaseExecutor::states_.size();i++){
    ...
    if(nIn > 0){
      ...
      uint_t ir;
      ir = (BaseExecutor::global_chunk_index_ + i) >> ((BaseExecutor::num_qubits_ - BaseExecutor::chunk_bits_));

      for(j=0;j<chunkSamples.size();j++){
        local_samples[vIdx[j]] = (ir << BaseExecutor::chunk_bits_) + chunkSamples[j];
      }
    }
  }
...
template <class densmat_t>
void ParallelExecutor<densmat_t>::apply_kraus(const reg_t &qubits,
...
#pragma omp parallel for 
    for(int_t ig=0;ig<BaseExecutor::num_groups_;ig++){
      for(int_t iChunk = BaseExecutor::top_chunk_of_group_[ig];iChunk < BaseExecutor::top_chunk_of_group_[ig + 1];iChunk++){
        BaseExecutor::states_[iChunk].qreg().apply_superop_matrix(qubits, Utils::vectorize_matrix(Utils::kraus_superop(kmats)));
      }
    }
  }
  else{
    for(int_t i=0;i<BaseExecutor::states_.size();i++)
      BaseExecutor::states_[i].qreg().apply_superop_matrix(qubits, Utils::vectorize_matrix(Utils::kraus_superop(kmats)));
  }
}
