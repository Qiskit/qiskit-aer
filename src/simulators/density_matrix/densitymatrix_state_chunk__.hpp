...
void State<densmat_t>::initialize_qreg(uint_t num_qubits,
  ...
  initialize_omp();
  BaseState::qregs_[0].set_num_qubits(num_qubits);
  BaseState::qregs_[0].initialize_from_data(state.data(), 1ULL << 2 * num_qubits);
}
...
void State<densmat_t>::initialize_qreg(uint_t num_qubits,
  ...
  initialize_omp();
  BaseState::qregs_[0].set_num_qubits(num_qubits);
  BaseState::qregs_[0].initialize_from_matrix(state);
}
...
void State<densmat_t>::initialize_qreg(uint_t num_qubits,
  ...
  initialize_omp();
  BaseState::qregs_[0].set_num_qubits(num_qubits);
  BaseState::qregs_[0].initialize_from_vector(state);
}

template <class densmat_t>
void State<densmat_t>::initialize_omp() {
  BaseState::qregs_[0].set_omp_threshold(omp_qubit_threshold_);
  if (BaseState::threads_ > 0)
    BaseState::qregs_[0].set_omp_threads(BaseState::threads_); // set allowed OMP threads in qubitvector
}
...
//  return BaseState::qregs_[0].required_memory_mb(2 * num_qubits);
...
//  BaseState::qregs_[0].set_json_chop_threshold(json_chop_threshold_);
    ...
    if (BaseState::cregs_[0].check_conditional(ops[iOp])) {
      switch (ops[iOp].type) {
        ...
        case Operations::OpType::matrix:
          apply_matrix(ops[iOp].qubits, ops[iOp].mats[0]);
          break;
        case Operations::OpType::diagonal_matrix:
          BaseState::qregs_[0].apply_diagonal_unitary_matrix(ops[iOp].qubits, ops[iOp].params);
          break;
        case Operations::OpType::superop:
          BaseState::qregs_[0].apply_superop_matrix(ops[iOp].qubits, Utils::vectorize_matrix(ops[iOp].mats[0]));
          break;
        case Operations::OpType::kraus:
          apply_kraus(ops[iOp].qubits, ops[iOp].mats);
          break;
        default:
...
void State<densmat_t>::apply_snapshot(const Operations::Op &op,
  ...
  switch (it -> second) {
    ...
    case Snapshots::cmemory:
      BaseState::snapshot_creg_memory(0, op, data);
      break;
    case Snapshots::cregister:
      BaseState::snapshot_creg_register(0, op, data);
      break;
...
void State<densmat_t>::snapshot_pauli_expval(const Operations::Op &op,
  ...
  for (const auto &param : op.params_expval_pauli) {
    ...
    expval += coeff * BaseState::qregs_[0].expval_pauli(op.qubits, pauli);
  }
...
void State<densmat_t>::apply_gate(const Operations::Op &op) {
  ...
  switch (it -> second) {
    ...
    case Gates::u1:
      ...
      break;
    case Gates::cx:
      BaseState::qregs_[0].apply_cnot(op.qubits[0], op.qubits[1]);
      break;
    case Gates::cz:
      BaseState::qregs_[0].apply_cz(op.qubits[0], op.qubits[1]);
      break;
    case Gates::id:
      break;
    case Gates::x:
      BaseState::qregs_[0].apply_x(op.qubits[0]);
      break;
    case Gates::y:
      BaseState::qregs_[0].apply_y(op.qubits[0]);
      break;
    case Gates::z:
      BaseState::qregs_[0].apply_z(op.qubits[0]);
      break;
    case Gates::h:
    ...
    case Gates::swap: {
      BaseState::qregs_[0].apply_swap(op.qubits[0], op.qubits[1]);
    } break;
    case Gates::ccx:
      BaseState::qregs_[0].apply_toffoli(op.qubits[0], op.qubits[1], op.qubits[2]);
      break;
    default:
...
void State<densmat_t>::apply_matrix(const reg_t &qubits, const cmatrix_t &mat) {
  if (mat.GetRows() == 1) {
    BaseState::qregs_[0].apply_diagonal_unitary_matrix(qubits, Utils::vectorize_matrix(mat));
  } else {
    BaseState::qregs_[0].apply_unitary_matrix(qubits, Utils::vectorize_matrix(mat));
  }
}

template <class densmat_t>
void State<densmat_t>::apply_gate_u3(uint_t qubit, double theta, double phi, double lambda) {
  BaseState::qregs_[0].apply_unitary_matrix(reg_t({qubit}), Utils::VMatrix::u3(theta, phi, lambda));
}

template <class densmat_t>
void State<densmat_t>::apply_gate_phase(uint_t qubit, complex_t phase) {
  cvector_t diag = {{1., phase}};
  BaseState::qregs_[0].apply_diagonal_unitary_matrix(reg_t({qubit}), diag);
}
...
void State<densmat_t>::apply_measure(const reg_t &qubits,
  ...
  BaseState::cregs_[0].store_measure(outcome, cmemory, cregister);
}

template <class densmat_t>
rvector_t State<densmat_t>::measure_probs(const reg_t &qubits) const {
  return BaseState::qregs_[0].probabilities(qubits);
}
...
std::vector<reg_t> State<densmat_t>::sample_measure(const reg_t &qubits,
  ...
  auto allbit_samples = BaseState::qregs_[0].sample_measure(rnds);

  // Convert to reg_t format
  std::vector<reg_t> all_samples;
  all_samples.reserve(shots);
  for (int_t val : allbit_samples) {
    reg_t allbit_sample = Utils::int2reg(val, 2, BaseState::qregs_[0].num_qubits());
    reg_t sample;
...
void State<densmat_t>::apply_reset(const reg_t &qubits) {
  ...
  BaseState::qregs_[0].apply_superop_matrix(qubits, Utils::vectorize_matrix(reset_op));
}
...
void State<densmat_t>::measure_reset_update(const reg_t &qubits,
  ...
  if (qubits.size() == 1) {
    ...
    BaseState::qregs_[0].apply_diagonal_unitary_matrix(qubits, mdiag);

    // If it doesn't agree with the reset state update
    if (final_state != meas_state) {
      BaseState::qregs_[0].apply_x(qubits[0]);
    }
  }
  // Multi qubit case
  else {
    ...
    if (final_state != meas_state) {
      ...
      BaseState::qregs_[0].apply_unitary_matrix(qubits, perm);
    }
  }
}
...
void State<densmat_t>::apply_kraus(const reg_t &qubits,
  ...
  BaseState::qregs_[0].apply_superop_matrix(qubits,
