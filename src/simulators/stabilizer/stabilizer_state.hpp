...
protected:
  ...
  */
  ...
  size_t max_qubits_snapshot_probs_ = 30;
...
void State::initialize_qreg(uint_t num_qubits,
  ...
  if (state.num_qubits() != num_qubits) {
    throw std::invalid_argument("Stabilizer::State::initialize: initial state "
...
void State::set_config(const json_t &config) {
  ...
  JSON::get_value(max_qubits_snapshot_probs_,
                  "stabilizer_max_snapshot_probabilities", config);
    ...
    if (BaseState::creg_.check_conditional(op)) {
      ...
      default:
        throw std::invalid_argument(
            "Stabilizer::State::invalid instruction \'" + op.name + "\'.");
      }
    }
  }
}
...
void State::apply_snapshot(const Operations::Op &op, ExperimentData &data) {
  ...
  if (it == snapshotset_.end())
    throw std::invalid_argument(
        "Stabilizer::State::invalid snapshot instruction \'" + op.name + "\'.");
  switch (it->second) {
...
void State::snapshot_stabilizer(const Operations::Op &op,
                                ExperimentData &data) {
  ...
  data.add_singleshot_snapshot("stabilizer", op.string_params[0],
                               clifford["stabilizers"]);
}

void State::snapshot_probabilities(const Operations::Op &op,
                                   ExperimentData &data, bool variance) {
