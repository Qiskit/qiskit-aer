...
public:
  ...
  template <typename list_t>
  void initialize_from_matrix(const uint_t iChunk, const list_t &mat);
  ...
  void save_creg(const uint_t iChunk, ExperimentResult &result,
  ...
  template <class T>
  void save_data_average(const uint_t iChunk, ExperimentResult &result,
  ...
  template <class T>
  void save_data_pershot(const uint_t iChunk, ExperimentResult &result,
                         const std::string &key, const T& datum,
  ...
  void apply_save_expval(const uint_t iChunk, const Operations::Op &op, ExperimentResult &result);
  ...
  void snapshot_state(const uint_t iChunk, const Operations::Op &op, ExperimentResult &result,
                      std::string name = "") const;

  // Snapshot the classical memory bits state (single-shot)
  void snapshot_creg_memory(const uint_t iChunk, const Operations::Op &op, ExperimentResult &result,
                            std::string name = "memory") const;

  // Snapshot the classical register bits state (single-shot)
  void snapshot_creg_register(const uint_t iChunk, const Operations::Op &op, ExperimentResult &result,
...
protected:
  ...
  template <typename InputIterator>
  void apply_cache_blocking_ops(const uint_t iChunk, InputIterator first,
                 InputIterator last,
                 ExperimentResult &result,
                 RngEngine &rng);
  ...
  virtual bool apply_batched_op(const uint_t iChunk, const Operations::Op &op,
                                ExperimentResult &result,
  ...
  //check conditional
  bool check_conditional(const uint_t iChunk, const Operations::Op &op);
  ...
  void block_diagonal_matrix(const uint_t iChunk, reg_t &qubits, cvector_t &diag);
  ...
  inline uint_t get_global_shot_index(const uint_t iChunk) const
  {
...
bool StateChunk<state_t>::allocate_qregs(uint_t num_chunks)
{
  ...
  for(uint_t i = 0; i < num_groups_; i++) {
...
#pragma omp parallel for num_threads(num_groups_) if(num_groups_ > 1 && chunk_omp_parallel_)
      for(uint_t ig = 0; ig < num_groups_; ig++)
...
template <class state_t>
template <typename InputIterator>
void StateChunk<state_t>::apply_cache_blocking_ops(const uint_t iChunk, InputIterator first,
               InputIterator last,
               ExperimentResult &result,
               RngEngine &rng)
{
...
template <class state_t>
bool StateChunk<state_t>::check_conditional(const uint_t iChunk, const Operations::Op &op)
{
  if(multi_shots_parallelization_){
    //multi-shots parallelization
    if(op.conditional){
...
#pragma omp parallel for if(num_groups_ > 1 && chunk_omp_parallel_)
    for (uint_t i = 0; i < num_groups_; i++) {
...
template <class state_t>
void StateChunk<state_t>::save_creg(const uint_t iChunk, ExperimentResult &result,
                               const std::string &key,
...
template <class state_t>
template <class T>
void StateChunk<state_t>::save_data_average(const uint_t iChunk, ExperimentResult &result,
                                       const std::string &key,
...
template <class state_t>
template <class T>
void StateChunk<state_t>::save_data_pershot(const uint_t iChunk, ExperimentResult &result,
                                       const std::string &key,
                                       const T& datum, OpType type,
                                       DataSubType subtype) const {
  int_t ishot = get_global_shot_index(iChunk);
...
template <class state_t>
void StateChunk<state_t>::snapshot_state(const uint_t iChunk, const Operations::Op &op,
                                    ExperimentResult &result,
                                    std::string name) const
{
...
template <class state_t>
void StateChunk<state_t>::snapshot_creg_memory(const uint_t iChunk, const Operations::Op &op,
                                          ExperimentResult &result,
                                          std::string name) const
{
  int_t ishot = get_global_shot_index(iChunk);
...
template <class state_t>
void StateChunk<state_t>::snapshot_creg_register(const uint_t iChunk, const Operations::Op &op,
                                            ExperimentResult &result,
                                            std::string name) const
{
  int_t ishot = get_global_shot_index(iChunk);
...
template <class state_t>
void StateChunk<state_t>::apply_save_expval(const uint_t iChunk, const Operations::Op &op,
                                       ExperimentResult &result){
...
//-------------------------------------------------------------------------
template <class state_t>
void StateChunk<state_t>::block_diagonal_matrix(const uint_t iChunk, reg_t &qubits, cvector_t &diag)
{
...
template <class state_t>
template <typename list_t>
void StateChunk<state_t>::initialize_from_vector(const uint_t iChunkIn, const list_t &vec)
{
  if(multi_chunk_distribution_){
#pragma omp parallel for if(chunk_omp_parallel_)
    for (uint_t iChunk = 0; iChunk < num_local_chunks_; iChunk++) {
...
template <class state_t>
template <typename list_t>
void StateChunk<state_t>::initialize_from_matrix(const uint_t iChunkIn, const list_t &mat)
{
  if(multi_chunk_distribution_){
#pragma omp parallel for if(chunk_omp_parallel_)
    for (uint_t iChunk = 0; iChunk < num_local_chunks_; iChunk++) {
...
auto StateChunk<state_t>::apply_to_matrix(bool copy)
{
  ...
  if(distributed_rank_ == 0){
    ...
    for (uint_t iChunk = 0; iChunk < num_global_chunks_; iChunk++) {
...
void StateChunk<state_t>::apply_chunk_swap(const reg_t &qubits)
{
  ...
  if(q1 < chunk_bits_*qubit_scale()){
    //inside chunk
#pragma omp parallel for num_threads(num_groups_) if(chunk_omp_parallel_ && num_groups_ > 1)
    for (uint_t ig = 0; ig < num_groups_; ig++)
...
#pragma omp parallel for if(chunk_omp_parallel_)
      for (uint_t iPair = 0; iPair < nPair; iPair++) {
        uint_t baseChunk;
        if(q0 < chunk_bits_*qubit_scale()){
...
#pragma omp parallel for if(chunk_omp_parallel_)
      for(uint_t iPair = 0; iPair < nPair; iPair++) {
        uint_t baseChunk = iPair & (mask-1);
        baseChunk += ((iPair - baseChunk) << 1);

        uint_t iChunk1 = baseChunk;
        uint_t iChunk2 = baseChunk | mask;
...
void StateChunk<state_t>::gather_state(std::vector<std::complex<data_t>>& state)
{
  ...
  if(distributed_procs_ > 1){
    ...
    if(distributed_rank_ == 0){
      ...
      offset = 0;
      for (uint_t i = 1; i < distributed_procs_; i++) {
        MPI_Status st;
        MPI_Recv(&size, 1, MPI_UINT64_T, i, i*2, distributed_comm_, &st);
        MPI_Recv(&state[offset], size*sizeof(std::complex<data_t>), MPI_BYTE, i, i*2+1, distributed_comm_, &st);
        offset += size;
      }
    }
    else{
      MPI_Send(&local_size, 1, MPI_UINT64_T, 0, myrank_*2, distributed_comm_);
      MPI_Send(&state[0], local_size*sizeof(std::complex<data_t>), MPI_BYTE, 0, myrank_*2+1, distributed_comm_);
    }
  }
#endif
}
  ...
  if(distributed_procs_ > 1){
    ...
    if(distributed_rank_ == 0){
      ...
      for(uint_t i=1;i<distributed_procs_;i++){
        MPI_Status st;
        MPI_Recv(&size, 1, MPI_UINT64_T, i, i*2, distributed_comm_, &st);
        MPI_Recv(state.data() + offset, size*sizeof(std::complex<data_t>), MPI_BYTE, i, i*2+1, distributed_comm_, &st);
        offset += size;
      }
    }
    else{
      MPI_Send(&local_size, 1, MPI_UINT64_T, 0, myrank_*2, distributed_comm_);
      MPI_Send(state.data(), local_size*sizeof(std::complex<data_t>), MPI_BYTE, 0, myrank_*2+1, distributed_comm_);
    }
  }
#endif
}
...
void StateChunk<state_t>::gather_creg_memory(void)
{
  ...
  uint_t n64 = (cregs_[0].memory_size() + 63) >> 6;

  reg_t bin_memory(n64*num_local_chunks_,0);
  //compress memory string to binary
#pragma omp parallel for
  for (uint_t i = 0; i < num_local_chunks_; i++) {
    for (uint_t j = 0; j < cregs_[0].memory_size(); j++) {
      uint_t i64 = j >> 6;
      uint_t ibit = j & 63;
...
#pragma omp parallel for
  for (uint_t i = 0; i < num_global_chunks_; i++) {
    for (uint_t j = 0; j < cregs_[0].memory_size(); j++) {
      uint_t i64 = j >> 6;
      uint_t ibit = j & 63;
