...
template <typename simulator_t, typename data_t = double>
class BaseAvx2 : public simulator_t {
public:
    ...
    BaseAvx2();
    explicit BaseAvx2(size_t num_qubits);
    virtual ~BaseAvx2(){};
    BaseAvx2(const BaseAvx2& obj) = delete;
    BaseAvx2 &operator=(const BaseAvx2& obj) = delete;
...
protected:
    ...
    using Base = simulator_t;

    virtual size_t calculate_num_threads();
};
...
template <typename simulator_t, typename data_t>
inline std::ostream &operator<<(std::ostream &out, const QV::BaseAvx2<simulator_t, data_t>&qv) {
...
//------------------------------------------------------------------------------
template <typename simulator_t, typename data_t>
BaseAvx2<simulator_t, data_t>::BaseAvx2(size_t num_qubits) {
  Base::num_qubits_ = 0;
  Base::data_ = nullptr;
  Base::checkpoint_ = 0;
...
template <typename simulator_t, typename data_t>
BaseAvx2<simulator_t, data_t>::BaseAvx2() : BaseAvx2(0) {}

template <typename simulator_t, typename data_t>
void BaseAvx2<simulator_t, data_t>::apply_matrix(const uint_t qubit,
...
template <typename simulator_t, typename data_t>
void BaseAvx2<simulator_t, data_t>::apply_matrix(const reg_t& qubits,
                                           const cvector_t<double>& mat) {
  if (apply_matrix_avx<data_t>(reinterpret_cast<data_t*>(Base::data_),
                               Base::data_size_, qubits.data(), qubits.size(),
                               reinterpret_cast<data_t *>(Base::convert(mat).data()),
...
template <typename simulator_t, typename data_t>
size_t BaseAvx2<simulator_t, data_t>::calculate_num_threads() {
