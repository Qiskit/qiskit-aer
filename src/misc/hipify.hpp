...
__device__ T __shfl_xor_aux(T var, int laneMask) {
 // Assert based on the values that make sense in CUDA.
  static_assert(mask == 0xffffffff, "Shuffle XOR implementation assumes all wavefront is active.");
  static_assert(width == 32, "Shuffle XOR implementation assumes on the whole wavefront.");
...
__device__ T __shfl_aux(T var, int lane) {
 // Assert based on the values that make sense in CUDA.
  static_assert(mask == 0xffffffff, "Shuffle implementation assumes all wavefront is active.");
  static_assert(width == 32, "Shuffle implementation assumes on the whole wavefront.");
  // In AMDGCN all wavefront intrinsics are synchronous.
  return __shfl(var, lane, _WS);
}
#define __shfl_sync(mask, var, lane, width) \
    __shfl_aux<mask, width>(var, lane);
