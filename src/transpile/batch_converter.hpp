...
const stringmap_t<ParamGates> BatchConverter::gateset_(
  {
    ...
    {"rzx", ParamGates::rzx},
    {"mcr", ParamGates::mcr},
    {"mcrx", ParamGates::mcrx},
    {"mcry", ParamGates::mcry},
    {"mcrz", ParamGates::mcrz},
    {"mcphase", ParamGates::mcp},
    {"mcp", ParamGates::mcp},
    {"mcu1", ParamGates::mcp},
    {"mcu2", ParamGates::mcu2},
    {"mcu3", ParamGates::mcu3},
    {"mcu", ParamGates::mcu}
  }
);
...
void BatchConverter::gate_to_matrix(Operations::Op& op, uint_t num_params) const
{
  ...
  uint_t matrix_size;
  if(it->second == ParamGates::mcrz || it->second == ParamGates::rzz || it->second == ParamGates::mcp){
    matrix_size = 2ull;
  ...
  auto store_matrix = [&matrix_array, matrix_size](int_t iparam, cvector_t mat)
  {
  ...
  case ParamGates::mcr:
    for(int_t i=0;i<num_params;i++)
      store_matrix(i, Linalg::VMatrix::r(op.params[i*2], op.params[i*2+1]));
    break;
  ...
  case ParamGates::mcu3:
    for(int_t i=0;i<num_params;i++)
      store_matrix(i, Linalg::VMatrix::u3(std::real(op.params[i*3]), std::real(op.params[i*3+1]), std::real(op.params[i*3+2])));
    break;
  case ParamGates::mcu:
    for(int_t i=0;i<num_params;i++)
      store_matrix(i, Linalg::VMatrix::u4(std::real(op.params[i*4]), std::real(op.params[i*4+1]), std::real(op.params[i*4+2]), std::real(op.params[i*4+3])));
    break;
  case ParamGates::mcu2:
    for(int_t i=0;i<num_params;i++)
      store_matrix(i, Linalg::VMatrix::u2(std::real(op.params[i*2]), std::real(op.params[i*2+1])));
    break;
  case ParamGates::mcp:
    for(int_t i=0;i<num_params;i++)
