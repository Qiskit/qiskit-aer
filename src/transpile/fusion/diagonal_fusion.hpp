...
public:
  DiagonalFusion(std::shared_ptr<FusionMethod> method_ = std::make_shared<FusionMethod>())
    : method(method_), max_qubit(method_->get_default_max_qubit() * 2), min_qubit(3), threshold(method_->get_default_threshold_qubit()), active(method_->support_diagonal()) { }
...
bool DiagonalFusion::is_diagonal_op(const op_t& op) const {
  ...
  if (op.type == Operations::OpType::gate) {
    if (op.name == "p" || op.name == "cp" || op.name == "u1" || op.name == "cu1" || op.name == "mcu1" || op.name== "rz" || op.name== "rzz")
      return true;
    if (op.name == "u3")
