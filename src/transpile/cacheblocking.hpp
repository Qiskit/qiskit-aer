...
protected:
  ...
  void insert_sim_op(std::vector<Operations::Op>& ops, std::string name, const reg_t& qubits) const;
...
void CacheBlocking::insert_sim_op(std::vector<Operations::Op>& ops, std::string name, const reg_t& qubits) const
{
  Operations::Op op;
...
bool CacheBlocking::split_pauli(const Operations::Op& op,const reg_t blockedQubits,std::vector<Operations::Op>& out,std::vector<Operations::Op>& queue) const
{
  ...
  if(qubits_in_chunk.size() > 0){
    ...
    //mapping swapped qubits
    for(uint_t i = 0; i < qubits_in_chunk.size(); i++) {
