...
//------------------------------------------------------------------------------
// Python NLOHMAN JSON Conversion
//------------------------------------------------------------------------------
 ...
 * @param js a nl::json object to contain converted type.
 * @param o is a python object to convert.
 */
void to_json(nl::json &js, const py::handle &o);

/**
 * Create a python object from a json
 * @param js a nl::json object
 * @param o is a reference to an existing (empty) python object
 */
void from_json(const nl::json &js, py::object &o);
 ...
 */
template <typename T>
nl::json numpy_to_json(py::array_t<T, py::array::c_style> arr);
 ...
 */
template <typename T>
nl::json numpy_to_json_1d(py::array_t<T, py::array::c_style> arr);
 ...
 */
template <typename T>
nl::json numpy_to_json_2d(py::array_t<T, py::array::c_style> arr);
 ...
 */
template <typename T>
nl::json numpy_to_json_3d(py::array_t<T, py::array::c_style> arr);
...
template <typename T>
nl::json JSON::numpy_to_json_1d(py::array_t<T, py::array::c_style> arr) {
...
template <typename T>
nl::json JSON::numpy_to_json_2d(py::array_t<T, py::array::c_style> arr) {
    py::buffer_info buf = arr.request();
    if (buf.ndim != 2) {
...
template <typename T>
nl::json JSON::numpy_to_json_3d(py::array_t<T, py::array::c_style> arr) {
    py::buffer_info buf = arr.request();
    if (buf.ndim != 3) {
...
template <typename T>
nl::json JSON::numpy_to_json(py::array_t<T, py::array::c_style> arr) {
    py::buffer_info buf = arr.request();
...
void std::to_json(nl::json &js, const py::handle &obj) {
...
void std::from_json(const nl::json &js, py::object &o) {
    if (js.is_boolean()) {
        ...
        py::dict obj;
        for (nl::json::const_iterator it = js.cbegin(); it != js.cend(); ++it)
        {
            py::object tmp;
