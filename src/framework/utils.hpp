...
std::string bin2hex(std::string str, bool prefix) {
  ...
  if (str.size() >= 2 && str[0] == '0' && str[1] == 'b') {
    str.erase(0, 2);
    if (str.empty())
      return prefix ? std::string("0x") : std::string();
  }
  ...
  while (i < n && str[i] == '0')
    ++i;
  if (i == n)
    return prefix ? std::string("0x0") : std::string("0");
  ...
  while (i + 3 < n) {
    unsigned v =
        ((unsigned)(str[i] - '0') << 3) | ((unsigned)(str[i + 1] - '0') << 2) |
        ((unsigned)(str[i + 2] - '0') << 1) | ((unsigned)(str[i + 3] - '0'));
    out.push_back(LUT[v]);
    i += 4;
  }
