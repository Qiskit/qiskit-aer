...
public:
  ...
  // c_average
  template <class T>
  void save_data_average(const ClassicalRegister &creg, const std::string &key,
  ...
  template <class T>
  void save_data_single(const ClassicalRegister &creg, const std::string &key,
                        const T &datum, OpType type);
  ...
  void save_data_pershot(const ClassicalRegister& creg,
                         ...
                         DataSubType subtype = DataSubType::list, const uint_t num_shots = 1);
...
void ExperimentResult::save_count_data(const ClassicalRegister &creg,
                                       bool save_memory) {
...
void ExperimentResult::save_data_average(const ClassicalRegister &creg,
  ...
  case DataSubType::c_average:
    ...
    break;
  default:
    throw std::runtime_error("Invalid average data subtype for data key: " +
                             key);
  }
...
void ExperimentResult::save_data_pershot(const ClassicalRegister& creg,
                                       ...
                                       const T& datum, OpType type,
                                       DataSubType subtype, const uint_t nshots) {
  switch (subtype) {
