...
namespace AER{
  namespace Test {
    ...
    TEST_CASE( "Sundials Cvode wrapper initialization from different container", "[sundials cvode]" ) {
      ...
      auto ode = CvodeWrapper<std::vector<complex_t>>(OdeMethod::ADAMS, rhs_in<std::vector<complex_t>>, y_a, 0.0);
      REQUIRE(ode.get_solution() == y);
    }
    ...
    TEST_CASE( "Sensitivities computation", "[sundials cvode]"){
      double par1 = -5;
      double par2 = -2;
      CvodeWrapper<std::vector<complex_t>>::rhsFuncType f = [&par1, &par2](double t, const std::vector<complex_t>& y_raw, std::vector<complex_t>& ydot_raw){
    ...
    TEST_CASE( "Sensitivities computation. Movement.", "[sundials cvode]"){
      double par = 5;
      CvodeWrapper<std::vector<complex_t>>::rhsFuncType f = [&par](double t, const std::vector<complex_t>& y_raw, std::vector<complex_t>& ydot_raw){
