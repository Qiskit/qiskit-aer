...
namespace AER {
  namespace ODE {
    namespace Test {
      namespace {
        template <typename T> void rhs_in(double t, const T &y_raw, T &ydot_raw) {
        ...
        template <typename T> void rhs_in_to_bind(double t, const T &y_raw, T &ydot_raw, double par) {
      ...
      TEST_CASE("Sensitivities computation. Movement.", "[sundials cvode]") {
        double par = 5;
        rhsFuncType<std::vector<complex_t>> f = [&par](double t, const std::vector<complex_t> &y_raw,
                                                       std::vector<complex_t> &ydot_raw) {
