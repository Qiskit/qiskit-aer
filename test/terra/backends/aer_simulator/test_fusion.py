...
class TestGateFusion(AerSimulatorTestCase):
    ...
    def test_fusion_verbose(self):
        ...
        circuit = transpile(
            self.create_statevector_circuit(), backend,
        ...
        with self.subTest(msg='verbose default'):
            backend.set_options(
                **self.fusion_options(enabled=True, threshold=1))
            result = backend.run(circuit, shots=shots).result()
            # Assert fusion applied succesfully
            self.assertSuccess(result)
            meta = self.fusion_metadata(result)
    ...
    def test_kraus_noise_fusion(self, method, device):
        ...
        backend = self.backend(
            method=method, device=device, noise_model=self.noise_model_kraus(),
            **fusion_options)
        circuit = transpile(
    ...
    def test_non_kraus_noise_fusion(self, method, device):
        ...
        fusion_options = self.fusion_options(enabled=True, threshold=1)
        backend = self.backend(
            method=method, device=device, noise_model=self.noise_model_depol(),
            **fusion_options)
        circuit = transpile(
            self.create_statevector_circuit(), backend, optimization_level=0)
        result = backend.run(circuit, shots=shots).result()
    ...
    def test_fusion_operations(self):
        ...
        backend = self.backend(method="statevector")
        circuit = transpile(
            circuit, backend, optimization_level=0)

        result_disabled = backend.run(
            circuit, **self.fusion_options(enabled=False, threshold=1)).result()
    ...
    def test_fusion_qv(self):
        ...
        options_disabled = self.fusion_options(
            enabled=False, threshold=1, verbose=True)
        result_disabled = backend.run(circuit, shots=shots, **options_disabled).result()
        meta_disabled = self.fusion_metadata(result_disabled)

        options_enabled = self.fusion_options(
            enabled=True, threshold=1, verbose=True)
