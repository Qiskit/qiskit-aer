...
from test.terra.reference.ref_snapshot_amplitudes import (
    ...
    snapshot_state_pre_measure_statevector_deterministic,
    snapshot_amplitudes_post_measure_statevector_deterministic,
    snapshot_amplitudes_circuits_nondeterministic,
    snapshot_amplitudes_counts_nondeterministic,
    snapshot_amplitudes_pre_measure_statevector_nondeterministic,
    snapshot_amplitudes_post_measure_statevector_nondeterministic)
...
class QasmSnapshotAmplitudesTests:
    ...
    def test_snapshot_amplitudes_pre_measure_det(self):
        ...
        label = "snap"
        counts_targets = snapshot_state_counts_deterministic(shots)
        amplitude_targets = snapshot_state_pre_measure_amplitudes_deterministic(
        )
        circuits = snapshot_state_circuits_deterministic(label,
                                                         'amplitudes',
                                                         post_measure=False)
        ...
        if method not in QasmSnapshotAmplitudesTests.SUPPORTED_QASM_METHODS:
            ...
            for j, circuit in enumerate(circuits):
                data = result.data(circuit)
                snaps = self.amplitude_snapshots(data, label)
                self.assertTrue(len(snaps), 1)
    ...
    def test_snapshot_amplitudes_pre_measure_nondet(self):
        ...
        label = "snap"
        counts_targets = snapshot_state_counts_nondeterministic(shots)
        amplitude_targets = snapshot_state_pre_measure_amplitudes_nondeterministic(
        )
        circuits = snapshot_state_circuits_nondeterministic(label,
                                                            'amplitudes',
                                                            post_measure=False)
    ...
    def test_snapshot_amplitudes_post_measure_det(self):
        ...
        label = "snap"
        counts_targets = snapshot_state_counts_deterministic(shots)
        amplitude_targets = snapshot_state_post_measure_amplitudes_deterministic(
        )
        circuits = snapshot_state_circuits_deterministic(label,
                                                         'amplitudes',
                                                         post_measure=True)
    ...
    def test_snapshot_amplitudes_post_measure_nondet(self):
        ...
        label = "snap"
        counts_targets = snapshot_state_counts_nondeterministic(shots)
        amplitude_targets = snapshot_state_post_measure_amplitudes_nondeterministic(
        )
        circuits = snapshot_state_circuits_nondeterministic(label,
                                                            'amplitudes',
                                                            post_measure=True)
