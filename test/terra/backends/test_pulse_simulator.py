...
class TestPulseSimulator(common.QiskitAerTestCase):
    ...
    def single_pulse_schedule(self, phi):
        """ Creates schedule for single pulse experiment
        Args:
            phi (float): drive phase (phi in Hamiltonian)
        Returns:
            schedule (pulse schedule): schedule for this experiment
        """
    ...
    def frame_change_schedule(self, phi, fc_phi, fc_dur1, fc_dur2):
        """ Creates schedule for frame change experiment. Does a pulse w/ phase phi of duration dur1,
        ...
        Returns:
            schedule (pulse schedule): schedule for frame change experiment
        """
         # drive pulse (just phase; omega1 included in Hamiltonian)
        phase = np.exp(1j*phi)
    ...
    def create_qobj(self, shots, omega0, omega1, omega, phi, meas_level, schedule_type=None,
        ...
        Args:
            ...
            "frame_change" or (fill in)
            fc_phi (float): frame change phase
            fc_dur1 (int): duration of first pulse in fc experiment
            fc_dur2 (int): duration of second pulse in fc experiment
        Returns:
    ...
    def test_frame_change(self):
        ...
        fc_qobj = self.create_qobj(shots=shots, omega0=omega0, omega1=omega1, omega=omega, phi=phi,
                                   meas_level=2, schedule_type="frame_change", fc_phi=fc_phi,
                                   fc_dur1=fc_dur1, fc_dur2=fc_dur2)
