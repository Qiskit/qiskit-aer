...
from qiskit_aer.pulse.de.DE_Methods import (
    ODE_Method,
    RK4,
    ScipyODE,
    QiskitZVODE,
...
class TestDE_Methods(QiskitAerTestCase):
    def setUp(self):
        ...
        self.Y = np.array([[0.0, -1j], [1j, 0.0]], dtype=complex)
    ...
    def test_QiskitZVODE_options_and_defaults(self):
        ...
        solver = QiskitZVODE(t0=0.0, y0=np.array([1.0]), rhs=self.rhs, options=options)
    ...
    def _test_variable_step_method(self, method_str):
        ...
        solver = ode_method(t0=0.0, y0=np.array(0.0), rhs={"rhs": rhs}, options=options)
