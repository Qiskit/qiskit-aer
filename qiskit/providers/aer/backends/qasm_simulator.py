...
class QasmSimulator(AerBackend):
    ...
    * ``fusion_threshold`` (int): Threshold that number of qubits must be greater
    ...
    def _method_basis_gates(self):
        ...
        if method in ['density_matrix', 'density_matrix_gpu', 'density_matrix_custatevec', 'density_matrix_thrust']:
            return sorted([
                'u1', 'u2', 'u3', 'u', 'p', 'r', 'rx', 'ry', 'rz', 'id', 'x',
                'y', 'z', 'h', 's', 'sdg', 'sx', 'sxdg', 't', 'tdg', 'swap', 'cx',
    ...
    def _custom_instructions(self):
        ...
        method = self._options.get('method', None)
        if method in ['statevector', 'statevector_gpu', 'statevector_custatevec', 'statevector_thrust']:
            return sorted([
    ...
    def _set_method_config(self, method=None):
        ...
        if method in ['statevector', 'statevector_gpu', 'statevector_custatevec', 'statevector_thrust']:
            ...
            n_qubits = MAX_QUBITS_STATEVECTOR
        elif method in ['density_matrix', 'density_matrix_gpu', 'density_matrix_custatevec', 'density_matrix_thrust']:
