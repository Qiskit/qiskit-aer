...
class AerSimulator(AerBackend):
    ...
    * ``blocking_enable`` (bool): This option enables parallelization 
       ...
       This option is only available for ``"statevector"``, ``"density_matrix"`` and ``"unitary"``
       (Default: False).
    ...
    * ``batched_shots_optimization`` (bool): This option enables multiple shots optimization for GPU.
       If the input circuit does not allow sampling measurment and requires multiple shots 
       (with noise models or intermediate measurements) this option can highly accelerate with GPUs.
       This option is only available for ``"statevector"``, ``"density_matrix"`` and ``"unitary"``
    ...
    * ``batched_shots_optimization_max_qubits`` (int): This option sets the maximum number of qubits
       that enables multiple shots optimization for GPU.
