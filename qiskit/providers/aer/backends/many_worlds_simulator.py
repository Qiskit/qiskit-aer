...
"""Contains a (slow) python many worlds simulator.

This is an adaptation of QasmSimulator.py from terra, and mostly based on the
split vector simulator written by Gilad Polak (shleef).
...
.. code-block:: python
    ...
    ManyWorldsSimulatorPy().run(qobj)
...
class ManyWorldsSimulator(AerBackend):
    ...
    DEFAULT_CONFIGURATION = {
        ...
        'url': 'https://github.com/Qiskit/qiskit-terra',  # UPDATE
        'simulator': True,
        'local': True,
        'conditional': True,
        'open_pulse': False,
    ...
    # Should be set to True for the many worlds simulator
    SHOW_FINAL_STATE = True

    # Class level variable to determine whether to split the statevector upon measurement
    # This should be set to True for the many worlds simulator
    SPLIT_STATES = True

    def __init__(self, configuration=None, provider=None):
        super().__init__(
            None,  # no controller for this sim, it's purely Python for now
            configuration=(
    ...
    def _validate_initial_statevector(self):
        ...
        if length != required_dim:
            raise AerError('initial statevector is incorrect length: ' +
    ...
    def _set_options(self, qobj_config=None, backend_options=None):
        ...
        if self._initial_statevector is not None:
            ...
            if round(norm, 12) != 1:
                raise AerError('initial statevector is not normalized: ' +
                                    'norm {} != 1'.format(norm))
    ...
    def get_statevector_tree(self, result, decimals=None):
        ...
        """
        try:
            tree = result.data()['statevector_tree']
        ...
        for i in range(op_idx, len(experiment.instructions)):
            ...
            # Check if measure
                ...
                if relation == '==':
                    ...
                    raise AerError('Invalid boolean function relation.')
                ...
                raise AerError(err_msg.format(backend, operation.name))
...
"""
The below code is borrowed from Terra, as a temporary workaround to make the
ManyWorldSimulator code above be self-contained, without the need to create
a dependency on Terra
"""
