...
def _uniform_transmon_hamiltonian_dict(num_transmons,
    ...
    hamiltonian_str += _u_channel_cr_terms(drive_symbol, edges, all_same_drive=True)
...
def _full_transmon_hamiltonian_dict(num_transmons,
                                    dim_transmons,
                                    transmon_freqs,
                                    anharm_freqs,
                                    drive_strengths,
    ...
    # construct individual qubit terms
    hamiltonian_str = _single_transmon_drift_terms(freq_symbol, anharm_symbol, num_transmons)
    hamiltonian_str += _drive_terms(drive_symbol, num_transmons)

    # construct two transmon terms
    hamiltonian_str += _exchange_coupling_terms(coupling_symbol, coupling_graph)
    hamiltonian_str += _u_channel_cr_terms(drive_symbol, cr_idx_dict)
    ...
    sorted_edges = _graph_to_sorted_list(coupling_graph)
    for idx1, idx2 in sorted_edges:
        key = coupling_symbol + str(idx1) + str(idx2)
        var_dict[key] = coupling_strength_dict[frozenset({idx1,idx2})]
...
def _exchange_coupling_terms(coupling_symbol, coupling_graph, all_same_coupling=False):
    ...
    edges = _graph_to_sorted_list(coupling_graph)
...
def _u_channel_cr_terms(drive_symbol, cr_idx_dict, all_same_drive=False):
    """Returns a list of u channel cr terms for a given cr_idx_dict

    Args:
        drive_symbol (str): base string to use for drive strengths
        cr_idx_dict (dict): dict with keys assumed to be tuples containing two integers (representing
    ...
    for key, val in cr_idx_dict.items():
        driven_qubit_idx.append(key[0])
        u_channel_idx.append(val)

    if all_same_drive:
        drive_str_list = [drive_symbol] * len(cr_idx_dict)
    else:
        drive_str_list = [drive_symbol + str(key[0]) for key, value in cr_idx_dict.items()]
...
"""
Helper functions for manipulating internal data types
"""

def _graph_to_sorted_list(graph):
    graph_list = []
    for edge in graph:
        edge_list = list(edge)
        edge_list.sort()
...
def _graph_to_cr_idx_dict(graph):
    cr_idx_dict = {}
    for idx1, idx2 in graph:
        cr_idx_dict[(idx1, idx2)] = len(cr_idx_dict)
        cr_idx_dict[(idx2, idx1)] = len(cr_idx_dict)
