...
class ODE_Method(ABC):
    ...
    def set_y(self, new_y, reset=True):
        ...
        if new_y is None:
            self._state_type_converter = None
        else:
            ...
            if self._rhs is not None:
                self.set_rhs(self._rhs, reset=False)
    ...
    def set_rhs(self, rhs=None, reset=True):
        ...
        self._rhs = rhs
        if self._state_type_converter is not None:
...
class BMDE_Method(ODE_Method):
    """Abstract wrapper class for a BMDE solving method.
    """

    def set_rhs(self, rhs=None, reset=True):
        """Same as ODE_Method but the generator takes precedence over the rhs
        """
...
class Expm(BMDE_Method):
    ...
    def integrate(self, tf):
        ...
        for k in range(steps):
            self._y = expm(generator(self.t + (h * k) + h / 2) * h) @ self._y
