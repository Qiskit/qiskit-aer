...
"""
Initialize qubit registers to desired arbitrary state specified by dictionary.
"""
...
class InitializeKet(Instruction):
    ...
    def __init__(self, params, num_qubits):
        ...
        if not math.isclose(sum(np.absolute(list(params.values())) ** 2), 1.0,
                            abs_tol=_EPS):
            raise QiskitError("Sum of amplitudes-squared does not equal one.")

        super().__init__("initialize_ket", num_qubits, 0, [list(item) for item in params.items()])
...
def initialize_ket(self, params, qubits):
    """Apply initialize to circuit."""
    if isinstance(qubits, list):
        num_qubits = len(qubits)
    else:
        qubits = [qubits]
    return self.append(InitializeKet(params, num_qubits), qubits)


QuantumCircuit.initialize_ket = initialize_ket
