...
Multiple shots are also distributed to multiple nodes when setting `device=GPU` and `batched_shots_optimization=True`. The results are distributed to each processes.
